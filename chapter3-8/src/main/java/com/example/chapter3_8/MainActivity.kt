...
import com.example.chapter3_8.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private var isMapInit = false //네이버 맵 객체가 준비되지 않았는데 사용하는걸 방지
    private var markers = emptyList<Marker>()
    private var restaurantListAdapter = RestaurantListAdapter {
        //리사이클러 뷰 클릭 시 카메라 움직임 반영
        collapseBottomSheet()
        moveCamera(it, 17.0)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.bottomSheetLayout.searchResultRecyclerView.apply {
            layoutManager = LinearLayoutManager(context) //this는 recyclerView
